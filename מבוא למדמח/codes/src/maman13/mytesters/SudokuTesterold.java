...
public class SudokuTesterold {
    ...
    public static void main(String[] args) {
        ...
        generatedValid(square3x3Arr);
        square3x3Arr[0][1].setXY(0,0, 8);
        square3x3Arr[0][1].setXY(1, 2, 4);
        Sudoku s4 = new Sudoku(square3x3Arr);
        generatedValid(square3x3Arr);
        square3x3Arr[1][0].setXY(0,0,4);
        square3x3Arr[1][0].setXY(0,1,7);
        Sudoku s5 = new Sudoku(square3x3Arr);

        System.out.print("Checking two numbers in the same row: ");
        if (!s4.isValid()) System.out.println(ANSI_GREEN + "Returned OK" + ANSI_RESET);
        else System.out.println(ANSI_RED + "Returned ERROR - Check validation for two numbers in the same row" + ANSI_RESET);

        System.out.print("Checking two numbers in the same column: ");
        if (!s5.isValid()) System.out.println(ANSI_GREEN + "Returned OK" + ANSI_RESET);
        else System.out.println(ANSI_RED + "Returned ERROR - Check validation for two numbers in the same column");

    }

    private static void generatedValid(Square3x3[][] s) {
        s[0][0] = eVals(new int[]{8,9,7,6,1,5,4,3,2});
        s[0][1] = eVals(new int[]{4,2,5,3,7,8,9,6,1});
        s[0][2] = eVals(new int[]{1,6,3,4,2,9,7,8,5});
        s[1][0] = eVals(new int[]{7,4,9,2,6,3,5,8,1});
        s[1][1] = eVals(new int[]{6,5,2,8,1,4,7,3,9});
        s[1][2] = eVals(new int[]{8,3,1,9,5,7,2,4,6});
        s[2][0] = eVals(new int[]{3,2,8,1,7,6,9,5,4});
        s[2][1] = eVals(new int[]{5,9,7,2,4,3,1,8,6});
        s[2][2] = eVals(new int[]{6,1,4,5,9,8,3,7,2});
    }
